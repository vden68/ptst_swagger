...
class SwaggerModel:
    ...
    def record_test_to_list(self, method, path, text):
        if not type(self.record_test_list) == list:
            self._record_test_list=[]
        self._record_test_list.append(TestsToSwagger(method=method, path=path, text=text))
        self._record_test_list.append(TestsToSwagger(method=method, path=path, text=text))
        print('_record_test_list=', self._record_test_list)
        r_test_dict = []
        for r_test in self._record_test_list:
